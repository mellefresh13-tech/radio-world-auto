...
class MainActivity : AppCompatActivity() {
    ...
    private val playerListener = object : Player.Listener {
        override fun onIsPlayingChanged(isPlaying: Boolean) {
            if (isPlaying) bufferingSinceMs = null
            updatePlayerButton()
        }
                ...
                Player.STATE_BUFFERING -> {
                    if (bufferingSinceMs == null) {
                        ...
                        retryHandler.postDelayed({
                            ...
                            if (player.playbackState == Player.STATE_BUFFERING && currentStation?.id == station.id && currentStreamIndex + 1 < station.streams.size) {
        ...
        catalogRepository.loadStations(limit = 200) { result ->
            result.onSuccess { stations ->
                if (stations.isNotEmpty()) {
                    ...
                    applyPersistedState()
                    currentStation = currentStation?.let { current -> catalog.firstOrNull { it.id == current.id } ?: catalog.firstOrNull() } ?: catalog.firstOrNull()
                    syncPlayerPlaylist()
    ...
    private fun saveCatalogCacheAsync() {
        val stations = catalog.toList(); val countries = remoteCountries.toList(); val genres = remoteGenres.toList()
        cacheExecutor.execute { catalogCacheStore.save(stations, countries, genres) }
    }
    ...
    private fun applyCarSafeArea() {
        ...
        if (profile.isCarReference) binding.root.setPadding(profile.carSafeInsetPx, binding.root.paddingTop, binding.root.paddingRight, binding.root.paddingBottom)
    }
    private fun setupNavigation() {
        ...
        styleNavigationButtons(); setActiveNav(R.id.navPlayer)
    }
    ...
    private fun configureImmersiveWindow() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= 30) {
            ...
            window.insetsController?.let { controller -> controller.hide(android.view.WindowInsets.Type.systemBars()); controller.systemBarsBehavior = android.view.WindowInsetsController.BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE }
        } else {
            @Suppress("DEPRECATION") window.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_FULLSCREEN or View.SYSTEM_UI_FLAG_HIDE_NAVIGATION or View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY or View.SYSTEM_UI_FLAG_LAYOUT_STABLE or View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION or View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
        }
    }
    private fun showScreen(title: String, content: () -> Unit) {
        setActiveNav(when (title) { "PLAYER" -> R.id.navPlayer; "COUNTRIES" -> R.id.navCountries; "GENRES" -> R.id.navGenres; "FAVORITES" -> R.id.navFavorites; "RECENT" -> R.id.navRecents; else -> R.id.navSearch }); content()
    }
    private fun setActiveNav(activeId: Int) {
        intArrayOf(R.id.navPlayer, R.id.navCountries, R.id.navGenres, R.id.navFavorites, R.id.navRecents, R.id.navSearch).forEach { id ->
            findViewById<Button>(id).apply {
                ...
                compoundDrawableTintList = ColorStateList.valueOf(getColor(if (active) R.color.auto_bg else R.color.auto_text_muted))
                alpha = 1f; setHorizontallyScrolling(true); ellipsize = android.text.TextUtils.TruncateAt.MARQUEE; marqueeRepeatLimit = -1; isSelected = true
            }
        }
    }
    ...
    private fun renderPlayer() {
        ...
        if (station == null) {
            val root = screenRoot(); root.addView(topBar("RADIO WORLD", "Ready to tune in", "Choose a station to start listening", R.drawable.ic_radio))
            val empty = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; setPadding(dp(24), dp(24), dp(24), dp(24)); setBackgroundResource(R.drawable.bg_card) }
            empty.addView(ImageView(this).apply { setImageResource(R.drawable.ic_radio); imageTintList = ColorStateList.valueOf(getColor(R.color.auto_accent)); setBackgroundResource(R.drawable.bg_icon_badge); scaleType = ImageView.ScaleType.CENTER }, LinearLayout.LayoutParams(dp(86), dp(86)))
            empty.addView(marqueeTextView("Your radio is ready", 25f, R.color.auto_text_main, true, false), LinearLayout.LayoutParams(-1, dp(38)))
            empty.addView(TextView(this).apply { text = "Browse the worldwide catalog or search directly for a station."; textSize = 13f; setTextColor(getColor(R.color.auto_text_muted)); gravity = Gravity.CENTER; maxLines = 2; includeFontPadding = false })
            val actions = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(0, dp(22), 0, 0) }
            actions.addView(actionButton("COUNTRIES", R.drawable.ic_globe) { showScreen("COUNTRIES") { renderCountries() } }, LinearLayout.LayoutParams(dp(170), dp(52)).apply { marginEnd = dp(8) })
            actions.addView(actionButton("SEARCH", R.drawable.ic_search) { showScreen("SEARCH") { renderSearch() } }, LinearLayout.LayoutParams(dp(170), dp(52))); empty.addView(actions)
            root.addView(empty, LinearLayout.LayoutParams(-1, 0, 1f)); binding.contentContainer.setScreenContent(root); return
        }
        val root = screenRoot()
        val favorite = iconButton(if (station.favorite) R.drawable.ic_star_filled else R.drawable.ic_star_outline, "Favorite") { station.favorite = !station.favorite; if (station.favorite) favoriteIds.add(station.id) else favoriteIds.remove(station.id); persistFavorites(); renderPlayer() }
        ...
        val logo = ImageView(this).apply { scaleType = ImageView.ScaleType.CENTER_INSIDE; setBackgroundResource(R.drawable.bg_logo); contentDescription = station.name + " logo"; setImageResource(R.drawable.ic_radio); imageTintList = ColorStateList.valueOf(getColor(R.color.auto_accent)); tag = station.id }
        playerLogoView = logo; loadStationLogo(station, logo); hero.addView(logo, LinearLayout.LayoutParams(dp(180), dp(180)).apply { marginEnd = dp(24) })
        val info = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }; info.addView(label("LIVE RADIO")); info.addView(marqueeTextView(station.name, 27f, R.color.auto_text_main, true), LinearLayout.LayoutParams(-1, dp(40)))
        info.addView(marqueeTextView(listOf(station.country, station.city, station.genre).filter { it.isNotBlank() }.joinToString("  •  "), 13f, R.color.auto_text_muted), LinearLayout.LayoutParams(-1, dp(28)))
        val track = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(16), dp(13), dp(16), dp(13)); setBackgroundResource(R.drawable.bg_surface) }
        val trackTitle = marqueeTextView(station.songTitle?.takeIf { it.isNotBlank() } ?: "Live broadcast", 21f, R.color.auto_text_main, true)
        val trackArtist = marqueeTextView(station.artist?.takeIf { it.isNotBlank() } ?: "Waiting for track metadata", 12f, R.color.auto_text_muted)
        playerTrackView = trackTitle; playerArtistView = trackArtist; track.addView(trackTitle, LinearLayout.LayoutParams(-1, dp(32))); track.addView(trackArtist, LinearLayout.LayoutParams(-1, dp(22))); info.addView(track, LinearLayout.LayoutParams(-1, dp(72)))
        ...
        val controls = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(dp(6), dp(8), dp(6), dp(8)) }
        ...
        playPauseIcon = (play as LinearLayout).getChildAt(0) as ImageView; playPauseLabel = (play as LinearLayout).getChildAt(1) as TextView; controls.addView(play, LinearLayout.LayoutParams(0, dp(78), 1.8f).apply { marginEnd = dp(8) })
        controls.addView(controlTile(R.drawable.ic_shuffle, "SHUFFLE") { catalog.randomOrNull()?.let { playStation(it) } }, LinearLayout.LayoutParams(0, dp(78), 1f)); root.addView(controls, LinearLayout.LayoutParams(-1, dp(94))); updatePlayerButton(); binding.contentContainer.setScreenContent(root)
    }
    ...
    private fun renderCountries(filter: String = "") {
        ...
        val root = screenRoot(); root.addView(topBar("BROWSE", "Countries", "${all.size} countries with available radio", R.drawable.ic_globe))
        ...
        root.addView(search, LinearLayout.LayoutParams(-1, dp(54)).apply { bottomMargin = dp(12) }); val columns = if (uiProfile.isLandscape) 3 else 2
        val recycler = RecyclerView(this).apply { layoutManager = GridLayoutManager(this@MainActivity, columns); adapter = CountryAdapter(all.filter { filter.isBlank() || it.name.contains(filter, true) }) { country -> loadAndRenderStations(country.name, country = country.code, onBack = { renderCountries(search.text.toString()) }) }; setPadding(0, 0, 0, dp(8)); clipToPadding = false }
        search.addTextChangedListener(SimpleTextWatcher { q -> (recycler.adapter as CountryAdapter).submitList(all.filter { q.toString().isBlank() || it.name.contains(q.toString(), true) }) }); root.addView(recycler, LinearLayout.LayoutParams(-1, 0, 1f)); binding.contentContainer.setScreenContent(root)
    }
    ...
    private fun renderSavedStations(ids: List<String>, title: String, columns: Int = 1) {
        val loaded = ids.mapNotNull { id -> catalog.find { it.id == id } ?: catalogCacheStore.findStation(id) }.toMutableList(); val missing = ids.filterNot { id -> loaded.any { it.id == id } }
        if (missing.isEmpty()) { renderStationList(title, loaded, { renderPlayer() }, columns = columns); return }
        setActiveNav(if (title.startsWith("FAVORITE")) R.id.navFavorites else R.id.navRecents); binding.contentContainer.removeAllViews(); val root = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL }; root.addView(titleBlock(title, "Loading saved stations...", if (title.startsWith("FAVORITE")) R.drawable.ic_star_filled else R.drawable.ic_history)); binding.contentContainer.setScreenContent(root)
        fun loadMissing(index: Int) {
            if (index >= missing.size) { applyPersistedState(); renderStationList(title, ids.mapNotNull { id -> catalog.find { it.id == id } }, { renderPlayer() }, columns = columns); return }
            ...
            catalogRepository.loadStation(missing[index]) { result -> result.onSuccess { station -> if (catalog.none { it.id == station.id }) catalog.add(station); saveCatalogCacheAsync(); ensureStationInPlaylist(station) }; loadMissing(index + 1) }
        }
        loadMissing(0)
    }
    private fun renderStationList(title: String, stations: List<Station>, onBack: () -> Unit, country: String? = null, genre: String? = null, canLoadMore: Boolean = false, columns: Int = 1) {
        val root = screenRoot(); root.addView(topBar("STATIONS", title, if (stations.isEmpty()) "No stations found" else "${stations.size} stations available", R.drawable.ic_list, listOf(iconButton(R.drawable.ic_skip_previous, "Back") { onBack() })))
        val stationAdapter = StationAdapter(stations, onPlay = { playStation(it) }, onFavorite = { it.favorite = !it.favorite; if (it.favorite) favoriteIds.add(it.id) else favoriteIds.remove(it.id); persistFavorites() })
        val recycler = RecyclerView(this).apply { layoutManager = GridLayoutManager(this@MainActivity, if (uiProfile.isLandscape) 2 else 1); adapter = stationAdapter; setPadding(0, 0, 0, dp(8)); clipToPadding = false }
    ...
    private fun renderSearch() {
        ...
        root.addView(input, LinearLayout.LayoutParams(-1, dp(56)).apply { bottomMargin = dp(12) }); val results = RecyclerView(this).apply { layoutManager = GridLayoutManager(this@MainActivity, if (uiProfile.isLandscape) 2 else 1) }
        val adapter = StationAdapter(emptyList(), onPlay = { playStation(it) }, onFavorite = { it.favorite = !it.favorite; if (it.favorite) favoriteIds.add(it.id) else favoriteIds.remove(it.id); persistFavorites(); results.adapter?.notifyDataSetChanged() }); results.adapter = adapter; root.addView(results, LinearLayout.LayoutParams(-1, 0, 1f)); if (uiProfile.useOnScreenKeypad) root.addView(buildSearchKeypad(input, adapter))
        input.addTextChangedListener(SimpleTextWatcher { text -> val query = text.toString().trim(); val requestId = ++searchRequestId; searchHandler.removeCallbacksAndMessages(null); if (query.isBlank()) adapter.submitList(emptyList()) else if (query.length < 2) updateSearchResults(query, adapter) else searchHandler.postDelayed({ catalogRepository.loadStations(query = query, limit = 50) { result -> if (requestId != searchRequestId) return@loadStations; result.onSuccess { stations -> catalog.addAll(stations.filter { station -> catalog.none { it.id == station.id } }); applyPersistedState(); adapter.submitList(stations) } } }, 250L) })
        binding.contentContainer.setScreenContent(root)
    }
    ...
    private fun playStation(station: Station) {
        currentStation = station; currentStreamIndex = 0; streamRetryCount = 0; bufferingSinceMs = null; retryHandler.removeCallbacksAndMessages(null); ensureStationInPlaylist(station)
        ...
        addRecentStation(station); showScreen("PLAYER") { renderPlayer() }
    }
    private fun playCurrentStream() {
        val player = controller ?: return; val station = currentStation ?: return
        if (station.streams.isEmpty()) { showPlayerState("STREAM UNAVAILABLE", "No working stream"); return }
        ensureStationInPlaylist(station); val index = (0 until player.mediaItemCount).firstOrNull { player.getMediaItemAt(it).mediaId == station.id } ?: return
        showPlayerState("CONNECTING...", "Opening stream " + (currentStreamIndex + 1)); player.replaceMediaItem(index, stationToMediaItem(station, currentStreamIndex)); player.seekTo(index, 0L); player.prepare(); player.play()
    }
    ...
    private fun syncPlayerPlaylist() {
        val player = controller ?: return; if (player.mediaItemCount > 0) return
        val items = catalog.filter { it.streams.isNotEmpty() }.distinctBy { it.id }.take(200).map { stationToMediaItem(it, 0) }; if (items.isNotEmpty()) player.setMediaItems(items, false)
    }
    private fun ensureStationInPlaylist(station: Station) {
        val player = controller ?: return; val exists = (0 until player.mediaItemCount).any { player.getMediaItemAt(it).mediaId == station.id }; if (!exists) player.addMediaItem(stationToMediaItem(station, 0))
    }
    private fun updateNowPlayingArtist(artist: String) {
        val station = currentStation ?: return; val updated = station.copy(artist = artist); currentStation = updated; catalog = catalog.map { if (it.id == updated.id) updated else it }.toMutableList(); playerArtistView?.text = artist; updateMarquee(playerArtistView); updateCurrentMediaMetadata(updated)
    }
    private fun updateCurrentMediaMetadata(station: Station) {
        val player = controller ?: return; val index = player.currentMediaItemIndex; if (index < 0 || index >= player.mediaItemCount) return; val current = player.getMediaItemAt(index); val metadata = current.mediaMetadata.buildUpon().setTitle(station.songTitle?.takeIf { it.isNotBlank() } ?: station.name).setDisplayTitle(station.songTitle?.takeIf { it.isNotBlank() } ?: station.name).setArtist(station.artist?.takeIf { it.isNotBlank() } ?: station.name).setAlbumTitle(station.name).setStation(station.name).setGenre(station.genre).setMediaType(MediaMetadata.MEDIA_TYPE_RADIO_STATION).build(); player.replaceMediaItem(index, current.buildUpon().setMediaMetadata(metadata).build())
    }
    private fun stationToMediaItem(station: Station, streamIndex: Int): MediaItem {
        val stream = station.streams.getOrNull(streamIndex) ?: station.streams.first(); val title = station.songTitle?.takeIf { it.isNotBlank() } ?: station.name; val artist = station.artist?.takeIf { it.isNotBlank() } ?: station.name
        return MediaItem.Builder().setMediaId(station.id).setUri(stream).setTag(station.id).setMediaMetadata(MediaMetadata.Builder().setTitle(title).setDisplayTitle(title).setArtist(artist).setAlbumTitle(station.name).setStation(station.name).setGenre(station.genre).setMediaType(MediaMetadata.MEDIA_TYPE_RADIO_STATION).build()).build()
    }
    private fun switchToNextStream(reason: String) {
        val station = currentStation ?: return; val player = controller ?: return; if (currentStreamIndex + 1 >= station.streams.size) return; currentStreamIndex++; showPlayerState(reason, "Opening stream " + (currentStreamIndex + 1)); val index = player.currentMediaItemIndex; player.replaceMediaItem(index, stationToMediaItem(station, currentStreamIndex)); player.prepare(); player.play()
    }
    private fun updateNowPlayingTitle(rawTitle: String) {
        val station = currentStation ?: return; val parts = rawTitle.split(" - ", limit = 2); val updated = if (parts.size == 2) station.copy(songTitle = parts[1].trim(), artist = parts[0].trim()) else station.copy(songTitle = rawTitle, artist = station.artist ?: station.name); currentStation = updated; catalog = catalog.map { if (it.id == updated.id) updated else it }.toMutableList(); playerTrackView?.text = updated.songTitle?.takeIf { it.isNotBlank() } ?: "Live broadcast"; playerArtistView?.text = updated.artist?.takeIf { it.isNotBlank() } ?: "Waiting for track metadata"; updateMarquee(playerTrackView); updateMarquee(playerArtistView); updateCurrentMediaMetadata(updated)
    }
    ...
    private fun FrameLayout.setScreenContent(view: View) { removeAllViews(); addView(view, FrameLayout.LayoutParams(-1, -1)) }
    ...
    private fun verticalText(title: String, subtitle: String, titleSize: Float, subtitleSize: Float): LinearLayout = LinearLayout(this).apply {
        ...
        addView(TextView(this@MainActivity).apply { text = title; textSize = titleSize; setTextColor(getColor(R.color.auto_text_main)); setTypeface(typeface, android.graphics.Typeface.BOLD); maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END; includeFontPadding = false }, LinearLayout.LayoutParams(-1, dp((titleSize + 10).toInt())))
        addView(TextView(this@MainActivity).apply { text = subtitle; textSize = subtitleSize; setTextColor(getColor(R.color.auto_text_muted)); maxLines = 1; ellipsize = android.text.TextUtils.TruncateAt.END; includeFontPadding = false }, LinearLayout.LayoutParams(-1, dp((subtitleSize + 8).toInt())))
    }
    ...
    private fun topBar(eyebrow: String, title: String, subtitle: String, icon: Int, right: List<View> = emptyList()): View = LinearLayout(this).apply {
        ...
        addView(ImageView(this@MainActivity).apply { setImageResource(icon); imageTintList = ColorStateList.valueOf(getColor(R.color.auto_accent)); setBackgroundResource(R.drawable.bg_icon_badge); scaleType = ImageView.ScaleType.CENTER }, LinearLayout.LayoutParams(dp(50), dp(50)).apply { marginEnd = dp(14) })
        addView(LinearLayout(this@MainActivity).apply {
            orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER_VERTICAL
            addView(TextView(this@MainActivity).apply { text = eyebrow; textSize = 9f; setTextColor(getColor(R.color.auto_accent)); setTypeface(typeface, android.graphics.Typeface.BOLD); letterSpacing = 0.12f; includeFontPadding = false })
            addView(marqueeTextView(title, 26f, R.color.auto_text_main, true), LinearLayout.LayoutParams(-1, dp(34)))
            addView(marqueeTextView(subtitle, 11f, R.color.auto_text_muted), LinearLayout.LayoutParams(-1, dp(18)))
        }, LinearLayout.LayoutParams(0, dp(58), 1f))
        right.forEach { addView(it, LinearLayout.LayoutParams(dp(50), dp(50)).apply { marginStart = dp(7) }) }
    }
    private fun marqueeTextView(value: String, size: Float, colorRes: Int, bold: Boolean = false, marquee: Boolean = true): TextView = TextView(this).apply {
        text = value; textSize = size; setTextColor(getColor(colorRes)); if (bold) setTypeface(typeface, android.graphics.Typeface.BOLD); gravity = Gravity.CENTER_VERTICAL; includeFontPadding = false; maxLines = 1; isSingleLine = true
        if (marquee) { setHorizontallyScrolling(true); ellipsize = android.text.TextUtils.TruncateAt.MARQUEE; marqueeRepeatLimit = -1; isSelected = true }
    }
    private fun updateMarquee(view: TextView?) { view?.apply { setHorizontallyScrolling(true); ellipsize = android.text.TextUtils.TruncateAt.MARQUEE; marqueeRepeatLimit = -1; isSelected = true } }
    ...
    private fun controlTile(iconRes: Int, text: String, accent: Boolean = false, click: () -> Unit): View = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL; gravity = Gravity.CENTER; setPadding(dp(4), dp(6), dp(4), dp(6)); setBackgroundResource(if (accent) R.drawable.bg_giant_play else R.drawable.bg_button); isClickable = true; isFocusable = true; contentDescription = text; setOnClickListener { click() }
        addView(ImageView(this@MainActivity).apply { setImageResource(iconRes); imageTintList = ColorStateList.valueOf(getColor(if (accent) R.color.auto_bg else R.color.auto_text_main)); scaleType = ImageView.ScaleType.CENTER }, LinearLayout.LayoutParams(dp(if (accent) 34 else 28), dp(if (accent) 34 else 28)))
        addView(TextView(this@MainActivity).apply { this.text = text; textSize = if (accent) 10f else 9f; setTextColor(getColor(if (accent) R.color.auto_bg else R.color.auto_text_main)); setTypeface(typeface, android.graphics.Typeface.BOLD); gravity = Gravity.CENTER; includeFontPadding = false; letterSpacing = 0.04f }, LinearLayout.LayoutParams(-1, dp(22)))
    }
    private fun buildSearchKeypad(input: EditText, adapter: StationAdapter): View {
        ...
        "QWERTYUIOPASDFGHJKLZXCVBNM".forEach { letter -> val b = keyButton(letter.toString()) { input.append(letter.toString()); updateSearchResults(input.text.toString(), adapter) }; grid.addView(b, GridLayout.LayoutParams().apply { width = dp(46); height = dp(44); setMargins(dp(2), dp(2), dp(2), dp(2)) }) }
    ...
    private fun loadStationLogo(station: Station, target: ImageView) {
        target.tag = station.id; target.setImageResource(R.drawable.ic_radio); target.imageTintList = ColorStateList.valueOf(getColor(R.color.auto_accent)); val url = station.logo?.trim().orEmpty(); if (url.isBlank()) return
        ImageLoader.load(url) { bitmap -> if (target.tag == station.id) { target.imageTintList = null; target.setImageBitmap(bitmap) } }
    }
    private fun flagFor(code: String): String {
        if (code.length != 2) return "🌐"; val upper = code.uppercase(); val first = Character.codePointAt(upper, 0); val second = Character.codePointAt(upper, 1)
        return String(Character.toChars(0x1F1E6 + first - 'A'.code)) + String(Character.toChars(0x1F1E6 + second - 'A'.code))
    }
    private val uiProfile: UiProfile get() = UiProfile.from(resources)
    private fun dp(value: Int): Int = (value * resources.displayMetrics.density).toInt()
    override fun onDestroy() {
        retryHandler.removeCallbacksAndMessages(null); searchHandler.removeCallbacksAndMessages(null); catalogRepository.close(); cacheExecutor.shutdownNow(); controller?.removeListener(playerListener); controllerFuture?.let(MediaController::releaseFuture); controller = null; super.onDestroy()
    }
    private class SimpleTextWatcher(private val onChanged: (CharSequence) -> Unit) : android.text.TextWatcher {
