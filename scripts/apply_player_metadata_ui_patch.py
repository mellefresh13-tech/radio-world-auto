...
s = replace_once(s, '} else showPlayerState("STREAM UNAVAILABLE", "No working stream")', '} else { playerOffline = true; playerReconnecting = false; showPlayerState("STREAM UNAVAILABLE", "No working stream") }')
...
s = replace_once(s, '''private fun updateNowPlayingTitle(rawTitle: String) { val station = currentStation ?: return; val parts = rawTitle.split(" - ", limit = 2); val updated = if (parts.size == 2) station.copy(songTitle = parts[1].trim(), artist = parts[0].trim()) else station.copy(songTitle = rawTitle, artist = station.artist ?: station.name); currentStation = updated; restoredStationId = updated.id; catalog = catalog.map { if (it.id == updated.id) updated else it }.toMutableList(); playerTrackView?.text = updated.songTitle?.takeIf { it.isNotBlank() } ?: "Live broadcast"; playerArtistView?.text = updated.artist?.takeIf { it.isNotBlank() } ?: "Waiting for track metadata"; updateMarquee(playerTrackView); updateMarquee(playerArtistView) }''', '''private fun updateNowPlayingTitle(rawTitle: String) { val station = currentStation ?: return; val parts = rawTitle.split(" - ", limit = 2); val updated = if (parts.size == 2) station.copy(songTitle = parts[1].trim(), artist = parts[0].trim()) else station.copy(songTitle = rawTitle); currentStation = updated; restoredStationId = updated.id; catalog = catalog.map { if (it.id == updated.id) updated else it }.toMutableList(); playerTrackView?.text = updated.songTitle?.takeIf { it.isNotBlank() } ?: "Live broadcast"; updateMarquee(playerTrackView); updateCurrentMediaMetadata(updated.artist, updated.songTitle) }''')
