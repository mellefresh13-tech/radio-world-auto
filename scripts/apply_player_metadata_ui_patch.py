...
# Existing metadata/status/startup/failover patch.
...
s = replace_once(s, '''val title = mediaMetadata.title?.toString()?.trim().orEmpty()
            ...
            var track = title
            if (artist.isBlank() && title.contains(" - ")) { val parts = title.split(" - ", limit = 2); artist = parts[0].trim(); track = parts[1].trim() }
            if (track.isNotBlank() && track != station.name) updateNowPlayingTitle(if (artist.isNotBlank()) "$artist - $track" else track)
...
# Remember the station that was playing immediately before the current one.
s = replace_once(s, 'private fun playStation(station: Station) { startupPlaybackRestored = true; failedStationIds.clear(); currentStation = station;', 'private fun playStation(station: Station) { startupPlaybackRestored = true; failedStationIds.clear(); if (currentStation?.id != station.id) previousStationId = currentStation?.id; currentStation = station;')
...
# Controls: Back | large Play/Pause | Shuffle. Shuffle is the forward/random action.
old_controls = 'val controls = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(dp(6), dp(8), dp(6), dp(8)) }; val play = controlTile(if (controller?.isPlaying == true) R.drawable.ic_pause else R.drawable.ic_play, if (controller?.isPlaying == true) "PAUSE" else "PLAY", true) { togglePlayPause() }; playPauseIcon = (play as LinearLayout).getChildAt(0) as ImageView; playPauseLabel = (play as LinearLayout).getChildAt(1) as TextView; controls.addView(play, LinearLayout.LayoutParams(0, dp(78), 1.8f).apply { marginEnd = dp(8) }); controls.addView(controlTile(R.drawable.ic_shuffle, "SHUFFLE") { catalog.randomOrNull()?.let { playStation(it) } }, LinearLayout.LayoutParams(0, dp(78), 1f)); root.addView(controls, LinearLayout.LayoutParams(-1, dp(94))); updatePlayerButton(); binding.contentContainer.setScreenContent(root)'
new_controls = 'val controls = LinearLayout(this).apply { orientation = LinearLayout.HORIZONTAL; gravity = Gravity.CENTER; setPadding(dp(6), dp(8), dp(6), dp(8)) }; controls.addView(controlTile(R.drawable.ic_back, "BACK") { returnToPreviousStation() }, LinearLayout.LayoutParams(0, dp(78), 1f).apply { marginEnd = dp(8) }); val play = controlTile(if (controller?.isPlaying == true) R.drawable.ic_pause else R.drawable.ic_play, if (controller?.isPlaying == true) "PAUSE" else "PLAY", true) { togglePlayPause() }; playPauseIcon = (play as LinearLayout).getChildAt(0) as ImageView; playPauseLabel = (play as LinearLayout).getChildAt(1) as TextView; controls.addView(play, LinearLayout.LayoutParams(0, dp(78), 1.8f).apply { marginEnd = dp(8) }); controls.addView(controlTile(R.drawable.ic_shuffle, "SHUFFLE") { catalog.randomOrNull()?.let { playStation(it) } }, LinearLayout.LayoutParams(0, dp(78), 1f)); root.addView(controls, LinearLayout.LayoutParams(-1, dp(94))); updatePlayerButton(); binding.contentContainer.setScreenContent(root)'
s = replace_once(s, old_controls, new_controls)
...
print("Player metadata/status/startup/failover/history/control patch applied.")
